...
body{background:#f1f5f9;color:#111;transition:0.3s}
...
.analyze-btn{width:150px;margin:auto;display:block;padding:10px;background:#3b82f6;color:#fff;border:none;border-radius:10px;position:relative}
.spinner{border:3px solid #f3f3f3;border-top:3px solid #fff;border-radius:50%;width:15px;height:15px;animation:spin 1s linear infinite;display:inline-block;margin-left:10px;vertical-align:middle}
...
<div id="registerBox" class="container hidden">
  <div class="card" style="max-width:400px;margin:auto">
    <h2>إنشاء حساب</h2>
    <input id="ruser" placeholder="اسم المستخدم">
...
<script>
let TOKEN="", USERNAME="", ALL_ERRORS=[]
function showToast(msg,color="#22c55e"){let t=document.getElementById("toast");t.innerText=msg;t.style.background=color;t.style.display="block";setTimeout(()=>t.style.display="none",3000)}
function toggleMode(){document.body.classList.toggle("dark")}
function logout(){location.reload()}

function goRegister(){loginBox.classList.add("hidden");registerBox.classList.remove("hidden")}
function goLogin(){registerBox.classList.add("hidden");loginBox.classList.remove("hidden")}

async function register(){
let f=new FormData();f.append("username",ruser.value);f.append("password",rpass.value)
let r=await fetch("/register",{method:"POST",body:f})
showToast("تم إنشاء الحساب");goLogin()
}

async function login(){
let f=new FormData();f.append("username",user.value);f.append("password",pass.value)
let r=await fetch("/login",{method:"POST",body:f})
let d=await r.json()
if(d.token){TOKEN=d.token;USERNAME=d.username;loginBox.classList.add("hidden");systemBox.classList.remove("hidden");welcomeUser.innerText="مرحبًا "+USERNAME}else{showToast("فشل تسجيل الدخول","#ef4444")}
}

// ====== RENDER ======
function render(errors){
let right=document.getElementById("right"), left=document.getElementById("left")
errors.sort((a,b)=>new Date(b.date||0)-new Date(a.date||0))
right.innerHTML=`<h4>${b1.value}</h4>`
left.innerHTML=`<h4>${b2.value}</h4>`
errors.filter(x=>x.branch==b1.value).forEach(x=>{right.innerHTML+=`<div class="error"><div>المبلغ: ${x.amount}</div><div>نوع المستند: ${x.doc||"-"}</div><div>التاريخ: ${x.date||"-"}</div><div>السبب: ${x.reason||"-"}</div></div>`})
errors.filter(x=>x.branch==b2.value).forEach(x=>{left.innerHTML+=`<div class="error"><div>المبلغ: ${x.amount}</div><div>نوع المستند: ${x.doc||"-"}</div><div>التاريخ: ${x.date||"-"}</div><div>السبب: ${x.reason||"-"}</div></div>`})
}
...
if(doc)filtered=filtered.filter(x=>(x.doc||"").toLowerCase().includes(doc))
if(amount)filtered=filtered.filter(x=>String(x.amount)===amount)
if(type)filtered=filtered.filter(x=>(x.reason||"").includes(type))
render(filtered)
}
function resetFilter(){document.getElementById("filterDoc").value="";document.getElementById("filterAmount").value="";document.getElementById("filterType").value="";render(ALL_ERRORS)}

// ====== UPLOAD ======
async function upload(){
if(!TOKEN){showToast("يجب تسجيل الدخول أولاً","#ef4444");return}
let btn=document.getElementById("analyzeBtn")
btn.disabled=true;btn.innerHTML=`جاري التحليل <span class="spinner"></span>`;btn.style.opacity="0.6"
let file1=document.getElementById("f1").files[0]
let file2=document.getElementById("f2").files[0]
if(!file1||!file2){showToast("اختار الملفين أولاً","#ef4444");btn.disabled=false;btn.innerText="تحليل";btn.style.opacity="1";return}
let f=new FormData();f.append("file1",file1);f.append("file2",file2);f.append("b1",b1.value);f.append("b2",b2.value)
try{
let r=await fetch("/analyze",{method:"POST",body:f,headers:{"Authorization":"Bearer "+TOKEN}})
if(r.status===401){showToast("انتهت الجلسة، سجل دخول مرة ثانية","#ef4444");logout();return}
if(!r.ok){let text=await r.text();console.log("❌ response:",text);showToast("خطأ في التحليل","#ef4444");btn.disabled=false;btn.innerText="تحليل";btn.style.opacity="1";return}
let d=await r.json();if(!d||!d.errors||!Array.isArray(d.errors)){console.log("❌ رد السيرفر غلط:",d);showToast("التحليل رجع بيانات غير صحيحة","#ef4444");btn.disabled=false;btn.innerText="تحليل";btn.style.opacity="1";return}
if(!d.counts)d.counts={}
ALL_ERRORS=d.errors
let c1=d.counts?.[b1.value]||0,c2=d.counts?.[b2.value]||0,totalErrors=ALL_ERRORS.length||1,p1=Math.round((c1/totalErrors)*100),p2=Math.round((c2/totalErrors)*100)
let stats=document.getElementById("stats")
stats.innerHTML=`<div class="stat"><span>${b1.value}</span><b>${c1}</b><span>عدد الأخطاء</span><small>${p1}%</small></div><div class="stat"><span>${b2.value}</span><b>${c2}</b><span>عدد الأخطاء</span><small>${p2}%</small></div>`
render(ALL_ERRORS)
showToast("تم التحليل ✔️")
}catch(e){console.error("❌ error:",e);showToast("حصل خطأ غير متوقع","#ef4444")}
btn.disabled=false;btn.innerText="تحليل";btn.style.opacity="1"
}

// ===== DOWNLOAD ======
function download(){fetch("/download",{headers:{"Authorization":"Bearer "+TOKEN}}).then(res=>res.blob()).then(blob=>{let url=URL.createObjectURL(blob);let a=document.createElement("a");a.href=url;a.download="report.xlsx";a.click()})}
</script>
</body>
</html>
    """
